# EDM v2 — Authentication: JWT, password hashing, current user dependency
...
def require_role(minimum_role: Role):
    """Dependency factory: returns a FastAPI dependency that checks role >= minimum_role.

    Usage:
        @router.delete("/{id}")
        async def delete(..., current_user: User = Depends(require_role(Role.ADMIN))):
            ...
    """
...
# Convenience dependencies (importable directly)
...
async def get_current_active_admin(current_user: User = Depends(get_current_user)) -> User:
    """Dependency: require ADMIN or OWNER role (backward-compat)."""
